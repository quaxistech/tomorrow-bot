...
// ============================================================
// BUG-EXCH-1 regression — verified empirically against Bitget mainnet 2026-05-09:
// Bitget hedge_mode close: `side` ВСЕГДА совпадает с `holdSide`, независимо от tradeSide.
//   close LONG  → side=buy  (попытка sell+close+long → Bitget code:22002)
//   close SHORT → side=sell (попытка buy+close+short  → Bitget code:22002)
// Это прямо противоположно one-way mode и интуитивному "close = opposite direction".
// Этот тест проверяет фактический JSON, отправляемый submitter'ом.
// ============================================================

namespace {

std::shared_ptr<exchange::bitget::BitgetFuturesOrderSubmitter>
make_submitter_for_test() {
    auto null_logger = logging::create_console_logger(logging::LogLevel::Critical);
    auto rest = std::make_shared<exchange::bitget::BitgetRestClient>(
        "https://api.bitget.com", "", "", "", null_logger, 5000);
    config::FuturesConfig fc;
    fc.product_type = "USDT-FUTURES";
    fc.margin_mode = "isolated";
    return std::make_shared<exchange::bitget::BitgetFuturesOrderSubmitter>(
        rest, null_logger, fc);
}

OrderRecord make_close_order(PositionSide ps, Side intent_side) {
    OrderRecord ord;
    ord.order_id = OrderId("CLOSE-001");
    ord.symbol = Symbol("BTCUSDT");
    ord.original_quantity = Quantity(0.0001);
    ord.price = Price(80000.0);
    ord.position_side = ps;
    ord.trade_side = TradeSide::Close;
    ord.side = intent_side;
    ord.order_type = OrderType::Market;
    ord.tif = TimeInForce::GoodTillCancel;
    return ord;
}
...
TEST_CASE("BUG-EXCH-1: close LONG produces side=buy in JSON", "[mapping][hedge_mode][regression]") {
    auto submitter = make_submitter_for_test();

    // Even if upstream incorrectly passes Side::Sell for close-long
    // (legacy or strategy bug), the submitter MUST produce side=buy.
    auto ord = make_close_order(PositionSide::Long, Side::Sell);  // upstream wrong
    auto js = boost::json::parse(submitter->build_place_order_json(ord)).as_object();
    REQUIRE(js.contains("side"));
    CHECK(std::string(js.at("side").as_string()) == "buy");
    CHECK(std::string(js.at("tradeSide").as_string()) == "close");

    // Correct upstream side
    auto ord2 = make_close_order(PositionSide::Long, Side::Buy);
    auto js2 = boost::json::parse(submitter->build_place_order_json(ord2)).as_object();
    CHECK(std::string(js2.at("side").as_string()) == "buy");
    CHECK(std::string(js2.at("tradeSide").as_string()) == "close");
}

TEST_CASE("BUG-EXCH-1: close SHORT produces side=sell in JSON", "[mapping][hedge_mode][regression]") {
    auto submitter = make_submitter_for_test();

    auto ord = make_close_order(PositionSide::Short, Side::Buy);  // upstream wrong
    auto js = boost::json::parse(submitter->build_place_order_json(ord)).as_object();
    CHECK(std::string(js.at("side").as_string()) == "sell");
    CHECK(std::string(js.at("tradeSide").as_string()) == "close");

    auto ord2 = make_close_order(PositionSide::Short, Side::Sell);
    auto js2 = boost::json::parse(submitter->build_place_order_json(ord2)).as_object();
    CHECK(std::string(js2.at("side").as_string()) == "sell");
    CHECK(std::string(js2.at("tradeSide").as_string()) == "close");
}

TEST_CASE("OrderMapping: open LONG produces side=buy", "[mapping][hedge_mode]") {
    auto submitter = make_submitter_for_test();
    OrderRecord ord;
    ord.order_id = OrderId("OPEN-001");
    ord.symbol = Symbol("BTCUSDT");
    ord.original_quantity = Quantity(0.0001);
    ord.price = Price(80000.0);
    ord.position_side = PositionSide::Long;
    ...
    auto js = boost::json::parse(submitter->build_place_order_json(ord)).as_object();
    CHECK(std::string(js.at("side").as_string()) == "buy");
    CHECK(std::string(js.at("tradeSide").as_string()) == "open");
}

TEST_CASE("OrderMapping: open SHORT produces side=sell", "[mapping][hedge_mode]") {
    auto submitter = make_submitter_for_test();
    OrderRecord ord;
    ord.order_id = OrderId("OPEN-002");
    ord.symbol = Symbol("BTCUSDT");
    ord.original_quantity = Quantity(0.0001);
    ord.price = Price(80000.0);
    ord.position_side = PositionSide::Short;
