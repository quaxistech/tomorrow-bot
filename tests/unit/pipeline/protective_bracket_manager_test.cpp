...
TEST_CASE("ProtectiveBracketManager: update_sl ставит новый перед отменой старого",
          "[bracket][edge-31]") {
    auto sub = std::make_shared<StubSubmitter>();
    auto qry = std::make_shared<StubQuery>();
    auto log = std::make_shared<TestLogger>();
    ...
    // trail SL ↑
    sub->next_exchange_id = "PL-SL2";
    REQUIRE(mgr.update_sl(Symbol("BTCUSDT"), PositionSide::Long, 49800.0));

    REQUIRE(sub->submit_calls == 1);  // новый SL поставлен
    REQUIRE(sub->cancel_calls == 1);  // старый SL отменён

    auto st = mgr.get_state(Symbol("BTCUSDT"), PositionSide::Long);
    REQUIRE(st->sl_price == 49800.0);
    REQUIRE(st->sl_order_id.get() == "PL-SL2");
    REQUIRE(st->sl_source == BracketSource::StandalonePlan);
}

TEST_CASE("ProtectiveBracketManager: update_sl не отменяет старый при ошибке нового",
          "[bracket][edge-31]") {
    auto sub = std::make_shared<StubSubmitter>();
    auto qry = std::make_shared<StubQuery>();
    auto log = std::make_shared<TestLogger>();
    ...
    mgr.on_position_opened(Symbol("BTCUSDT"), PositionSide::Long,
                            50000.0, 0.01, 0.0, 49500.0);

    sub->submit_success = false;
    REQUIRE_FALSE(mgr.update_sl(Symbol("BTCUSDT"), PositionSide::Long, 49800.0));

    REQUIRE(sub->submit_calls == 1);
    REQUIRE(sub->cancel_calls == 0);  // старый НЕ отменён

    auto st = mgr.get_state(Symbol("BTCUSDT"), PositionSide::Long);
    REQUIRE(st->sl_price == 49500.0);  // не изменилось
}
