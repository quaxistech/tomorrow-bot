...
// Scalping refactor 2026-05: leverage_engine.uncertainty_multiplier was
// neutralised for Low/Moderate/High (returns 1.0) because uncertainty is
// already applied via uncertainty.size_multiplier in the pipeline allocator.
// Repeating the same signal on leverage compounded the suppression and pushed
// notional below min_notional on a small account.
TEST_CASE("Leverage: uncertainty_multiplier neutral except Extreme", "[leverage][uncertainty]") {
    auto cfg = make_default_config();
    LeverageEngine engine(cfg);
    ...
    SECTION("Moderate → 1.0 (no double-counting)") {
        auto ctx = make_default_context();
    ...
    SECTION("High → 1.0 (no double-counting)") {
        auto ctx = make_default_context();
        ctx.uncertainty = UncertaintyLevel::High;
        auto d = engine.compute_leverage(ctx);
        CHECK_THAT(d.uncertainty_factor, WithinAbs(1.0, 0.01));
    }

    SECTION("Extreme → 0.6 (crisis dampening only)") {
        auto ctx = make_default_context();
        ctx.uncertainty = UncertaintyLevel::Extreme;
        auto d = engine.compute_leverage(ctx);
        CHECK_THAT(d.uncertainty_factor, WithinAbs(0.6, 0.02));
    }
}
