...
> make_deps() {
    ...
    return {health, logger, metrics, clk};
}

TEST_CASE("Supervisor — начальное состояние Initializing", "[supervisor]") {
    auto [health, logger, metrics, clk] = make_deps();
    Supervisor sv(health, logger, metrics, clk);
