...
**Версия**: 0.5.0 | **Статус**: Pre-production (hardening) | **Тесты**: 602 тестов
...
**49+ модулей** · **9 стратегий** · **27 проверок риска** · **10 состояний FSM ордера** · **HTF тренд-фильтр** · **6 ML-модулей** · **Smart TWAP** · **Trailing Stop**
...
```bash
cd build-check
ctest --output-on-failure -j8   # 602 тестов
```
...
27. **Spot SELL without position** (v0.5: запрет продажи без открытой long-позиции)
...
- 602 тестов (было 559): +6 regression на reduce_position, +31 config validator, +3 spot-semantic risk, +3 risk regression
...
│   ├── common/                 # StrongType, Result, Error (25 кодов ошибок), перечисления
...
│   ├── ml/                     # ML-модули (6 модулей, 14 файлов)
...
│   ├── risk/                   # **27 проверок**, режимно-адаптивные лимиты, kill switch
...
├── tests/                      # 602 тестов (unit + integration + scenario)
