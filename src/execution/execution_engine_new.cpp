    ...
    const uncertainty::UncertaintySnapshot& uncertainty)
{
    ...
    // RAII-guard для cash reservation: гарантирует release при exception на любом
    // этапе (update_order, submit, post-submit registry calls). Production-hardening:
    // bad_alloc/disk-full/любые exceptions → margin не утекает.
    struct CashReservationGuard {
        portfolio::IPortfolioEngine* portfolio;
        OrderId order_id;
        bool needs_release;
        bool active{true};
    ...
    try {
        ...
        // CashGuard сбросит margin при unwind. Ордер остаётся в PendingAck для recovery.
        throw;
    }
    ...
    // Успешно отправлен — биржа знает об ордере.
    // CRITICAL: с этого момента cash_guard НЕ должен релизить margin (закрытие позиции
    // освободит cash через portfolio close path). Дезактивируем guard.
    cash_guard.active = false;

    // Все update'ы registry после успешного submit обёрнуты в try/catch:
    // если bad_alloc/disk-full здесь — ордер на бирже остаётся, но local registry
    // может быть в degraded состоянии. Reconciliation восстановит state.
    try {
        order.exchange_order_id = submit_result.exchange_order_id;
        registry_.update_order(order);
        registry_.record_intent(dedup_key);
    } catch (const std::exception& e) {
        logger_->critical("Execution",
            "Exception после успешного submit — ордер на бирже, local registry degraded",
            {{"order_id", order_id_str},
             {"exchange_id", submit_result.exchange_order_id.get()},
             {"what", e.what()}});
        // Не маскируем exception: pipeline должен видеть, что что-то аномальное.
        throw;
    }
