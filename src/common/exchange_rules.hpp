...
struct ExchangeSymbolRules {
    ...
    // EDGE-30-DYN (run61 2026-05-15): per-symbol max leverage от биржи. Bitget allow varies:
    // BTC до 125x, top altcoins до 75x, exotic — 20x-50x. 0 = не задан (используется config).
    double max_leverage{0.0};
    double taker_fee_rate{0.0};   ///< per-symbol taker fee от биржи, 0 = не задан (config fallback)
