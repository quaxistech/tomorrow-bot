...
struct RiskConfig {
    double max_position_notional{10000.0};  ///< Максимальный номинальный объём позиции (USD, абсолютный пол)
    double max_position_notional_pct{0.0};  ///< Макс. номинал как доля от total_capital (0=отключено, >0 auto-scales с депозитом и overrides абсолютный, если выше)
...
struct PairSelectionConfig {
    ...
    double rotation_interval_hours{24.0};        ///< Интервал ротации (часы, fractional OK)
