 ...
 * @brief Реализация YAML загрузчика конфигурации (D5: yaml-cpp)
 *
 * Парсер использует yaml-cpp 0.8+ для корректного разбора любых YAML 1.2 структур
 * (nested mappings, sequences, anchors, multi-document). Сохраняет совместимый с
 * оригинальным парсером flat-key API: рекурсивно сплющивает дерево в
 * `unordered_map<"section.subsection.key", "value">` и сериализует sequences
 * в comma-separated строки (формат `parse_list` в downstream-коде).
 */
...
/// Сериализация YAML scalar в строку (унифицирует bool/int/double/string).
std::string scalar_to_string(const YAML::Node& node) {
    if (!node.IsScalar()) return {};
    return node.Scalar();
}

/// Рекурсивный обход YAML дерева: выкладываем все scalars в flat map.
/// Sequences → comma-separated string (совместимо с downstream `parse_list`).
/// Maps → recursive walk с composed key "parent.child".
void flatten_yaml(const YAML::Node& node,
                  const std::string& prefix,
                  std::unordered_map<std::string, std::string>& out) {
    if (!node.IsDefined() || node.IsNull()) {
        if (!prefix.empty()) {
            out.emplace(prefix, std::string{});
        }
        return;
    }

    if (node.IsScalar()) {
        out[prefix] = scalar_to_string(node);
        return;
    }

    if (node.IsSequence()) {
        std::string joined;
        joined.reserve(node.size() * 8);
        for (std::size_t i = 0; i < node.size(); ++i) {
            const auto& child = node[i];
            if (i > 0) joined.push_back(',');
            if (child.IsScalar()) {
                joined += scalar_to_string(child);
            } else {
                // Nested map/sequence внутри списка — emit YAML emitter.
                YAML::Emitter em;
                em << child;
                joined += em.c_str() ? em.c_str() : "";
            }
        }
    ...
    if (node.IsMap()) {
        for (const auto& kv : node) {
            ...
            flatten_yaml(kv.second, full_key, out);
        }
        return;
    }
}
...
YamlConfigLoader::parse_yaml_flat(std::string_view content) {
    ...
    try {
        ...
        flatten_yaml(root, std::string{}, result);
    } catch (const YAML::Exception& e) {
        // Сигнализируем через специальный sentinel-ключ для верхнего уровня.
        // Загрузчик всегда проверяет наличие через `__yaml_parse_error`.
        result["__yaml_parse_error"] = e.what() ? e.what() : "yaml-cpp parse error";
    }
    return result;
}
...
Result<AppConfig> YamlConfigLoader::load(std::string_view path) {
    ...
    // Парсим YAML в плоскую карту (D5: yaml-cpp 0.8+)
    auto kv = parse_yaml_flat(content);

    // D5: yaml-cpp parse error → fail-fast. Sentinel установлен в parse_yaml_flat.
    if (auto it = kv.find("__yaml_parse_error"); it != kv.end()) {
        std::cerr << "[ConfigLoader] YAML parse error: " << it->second << "\n";
        return Err<AppConfig>(TbError::ConfigLoadFailed);
    }
    ...
    cfg.risk.max_position_notional_pct = parse_double(get_tracked( "risk.max_position_notional_pct", "0.0"), 0.0, "risk.max_position_notional_pct");
    ...
    cfg.pair_selection.rotation_interval_hours = parse_double(rotation_str, 24.0, "pair_selection.rotation_interval_hours");
