...
struct LiquidityFeatures {
    ...
    // EDGE-31: дополнительные depth metrics для scalp
    double book_depth_0_5pct_usdt{0.0};   // ±0.5% from mid
    double book_depth_1pct_usdt{0.0};     // ±1.0% from mid
...
struct VolatilityFeatures {
    ...
    // EDGE-31 scalp-specific vol
    double realized_vol_1m_pct{0.0};   // вычислено по 1m candles если доступны
    double realized_vol_5m_pct{0.0};
    double vol_quality_score{0.0};    // peak score @ vol_sweet_spot
    double score{0.0};                // normalized 0-1
};
...
struct OrderBookFeatures {
    ...
    // EDGE-31 scalp-specific
    double slippage_at_10usdt_bps{0.0};   // estimated slippage walking ask for $10
    double slippage_at_50usdt_bps{0.0};
    double resiliency_score{0.0};         // 0-1, насколько book устойчив
    double score{0.0};                // normalized 0-1
};
...
struct TrendQualityFeatures {
    ...
    // EDGE-31 candle microstructure (scalp-quality)
    double wick_ratio{0.0};            // 0-1, average wick/range over last N
    double body_to_range_ratio{0.0};   // 0-1, decisive candles indicator
    double trade_count_1m{0.0};        // estimated from volume_per_candle/avg_trade_size (best effort)
    double score{0.0};                // normalized 0-1
};
...
/// EDGE-31 Regime tag — какая стратегия подходит паре
enum class RegimeTag {
    Unknown,
    Momentum,        ///< Сильный тренд → MomentumContinuation
    MeanReversion,   ///< Boundary chop → BB-edge fade
    Breakout,        ///< Compression → squeeze breakout
    Avoid            ///< Не подходит для scalp
};

inline const char* regime_tag_name(RegimeTag t) {
    switch (t) {
        case RegimeTag::Momentum: return "Momentum";
        case RegimeTag::MeanReversion: return "MeanReversion";
        case RegimeTag::Breakout: return "Breakout";
        case RegimeTag::Avoid: return "Avoid";
...
struct SymbolScore {
    ...
    // EDGE-31 scalp-specific
    double scalpability_score{0.0};
    RegimeTag regime_tag{RegimeTag::Unknown};
    std::vector<std::string> hard_gate_failures;
