...
LiquidityFeatures FeatureCalculator::compute_liquidity(const MarketSnapshot& s) const {
    ...
    // EDGE-31: depth_0_5pct и depth_1pct для scalp slippage estimation
    if (mid > 0.0) {
        double lo_05 = mid * 0.995;
        double hi_05 = mid * 1.005;
        double lo_1 = mid * 0.99;
        double hi_1 = mid * 1.01;
        for (const auto& lvl : ob.bids) {
            if (lvl.price >= lo_05) liq.book_depth_0_5pct_usdt += lvl.quantity * lvl.price;
            if (lvl.price >= lo_1) liq.book_depth_1pct_usdt += lvl.quantity * lvl.price;
        }
        for (const auto& lvl : ob.asks) {
            if (lvl.price <= hi_05) liq.book_depth_0_5pct_usdt += lvl.quantity * lvl.price;
            if (lvl.price <= hi_1) liq.book_depth_1pct_usdt += lvl.quantity * lvl.price;
        }
    }
...
VolatilityFeatures FeatureCalculator::compute_volatility(const MarketSnapshot& s) const {
    ...
    if (vol.vol_to_spread_ratio > 1e-9) {
        ...
        double log_optimal = std::log(7.0);
        double sigma = 0.8;
        vol.score = std::exp(-(log_ratio - log_optimal) * (log_ratio - log_optimal)
                             / (2.0 * sigma * sigma));
    }

    // EDGE-31 Scalp-specific: realized_vol_1m/5m + vol_quality_score (peak @ sweet spot)
    // realized_vol на candle close-to-close stddev. Если candles 5m, реально это 5m vol;
    // 1m approximation = 5m / sqrt(5).
    if (vol.realized_vol_pct > 0.0) {
        // Bitget scanner candles обычно 5m по cfg.candle_interval.
        // Если candle_interval="1m" → realized_vol_1m == realized_vol_pct.
        // Если "5m" — adjustments via sqrt scaling.
        vol.realized_vol_5m_pct = vol.realized_vol_pct;
        vol.realized_vol_1m_pct = vol.realized_vol_pct / std::sqrt(5.0);
    }

    // Vol quality score — peak @ sweet_spot (default 0.30%/1m)
    const double sweet = cfg_.scalping.vol_sweet_spot_pct;
    if (vol.realized_vol_1m_pct > 0.0 && sweet > 0.0) {
        double dist = std::abs(vol.realized_vol_1m_pct - sweet) / sweet;
        vol.vol_quality_score = std::max(0.0, 1.0 - dist);
    }
...
OrderBookFeatures FeatureCalculator::compute_orderbook(const MarketSnapshot& s) const {
    ...
    double depth_component = std::min(total_5 / 100'000.0, 1.0);
    ...
    // EDGE-31 Slippage estimate — walk through asks для target_notional, симулируя taker buy.
    // bps = (avg_fill_price - mid) / mid × 10000
    auto estimate_slippage_bps = [&](double target_usdt) -> double {
        if (ob.asks.empty()) return 1e6;  // unrejectable signal
        double mid = ob.mid_price();
        if (mid <= 0.0) return 1e6;
        double remaining = target_usdt;
        double cost = 0.0, qty_filled = 0.0;
        for (const auto& lvl : ob.asks) {
            double level_value = lvl.quantity * lvl.price;
            if (level_value <= 0.0) continue;
            if (remaining <= level_value) {
                qty_filled += remaining / lvl.price;
                cost += remaining;
                remaining = 0.0;
                break;
            }
            qty_filled += lvl.quantity;
            cost += level_value;
            remaining -= level_value;
        }
        if (qty_filled <= 0.0 || remaining > 0.0) return 1e6;  // insufficient depth
        double avg_fill = cost / qty_filled;
        return (avg_fill - mid) / mid * 10000.0;
    };
    obf.slippage_at_10usdt_bps = estimate_slippage_bps(10.0);
    obf.slippage_at_50usdt_bps = estimate_slippage_bps(50.0);

    // EDGE-31 Resiliency — насколько book устойчив к take $50.
    // Approximation: fraction of book_depth_1pct остающейся после take.
    {
        double mid = ob.mid_price();
        if (mid > 0.0) {
            double depth_1pct = 0.0;
            double lo = mid * 0.99;
            double hi = mid * 1.01;
            for (const auto& lvl : ob.bids) if (lvl.price >= lo) depth_1pct += lvl.quantity * lvl.price;
            for (const auto& lvl : ob.asks) if (lvl.price <= hi) depth_1pct += lvl.quantity * lvl.price;
            constexpr double kSimTakeUsdt = 50.0;
            if (depth_1pct > kSimTakeUsdt) {
                obf.resiliency_score = std::clamp((depth_1pct - kSimTakeUsdt) / depth_1pct, 0.0, 1.0);
            }
        }
    }
...
TrendQualityFeatures FeatureCalculator::compute_trend_quality(const MarketSnapshot& s) const {
    ...
    // EDGE-31 Candle microstructure: wick_ratio + body_to_range_ratio + trade_count_1m estimate
    {
        double sum_wick_ratio = 0.0;
        double sum_body_ratio = 0.0;
        ...
        for (size_t i = start; i < candles.size(); ++i) {
            ...
            sum_wick_ratio += (upper_wick + lower_wick) / range;
            sum_body_ratio += body / range;
            sum_volume += c.volume;
            ++valid;
        }
        if (valid > 0) {
            tq.wick_ratio = sum_wick_ratio / valid;
            tq.body_to_range_ratio = sum_body_ratio / valid;
            // Trade count 1m approximation: volume per candle / typical trade size.
            // Без trades data — proxy: candle volume (USDT) / 100 (assume $100 avg trade).
            double avg_vol_per_candle = sum_volume / valid;
            double mid = s.orderbook.mid_price();
            if (mid > 0.0) {
                double usdt_per_candle = avg_vol_per_candle * mid;
                double interval_min = 5.0;  // candle_interval default "5m"
                tq.trade_count_1m = (usdt_per_candle / 100.0) / interval_min;
            }
        }
    }
