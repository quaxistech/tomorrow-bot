...
FilterVerdict PairFilter::evaluate(const MarketSnapshot& snapshot,
    ...
    // EDGE-31 Scalping hard gates — cliff cutoffs для scalp executability.
    // Активируются только если scalping.enabled = true. Reject если ANY fail.
    if (cfg_.scalping.enabled) {
        const auto& sp = cfg_.scalping;
        if (features.spread.spread_bps > sp.hg_max_spread_bps) {
            return {FilterReason::WideSpread,
                    "scalp_gate: spread=" + std::to_string(static_cast<int>(features.spread.spread_bps)) +
                    "bps > max=" + std::to_string(static_cast<int>(sp.hg_max_spread_bps))};
        }
        if (features.liquidity.book_depth_1pct_usdt > 0.0 &&
            features.liquidity.book_depth_1pct_usdt < sp.hg_min_book_depth_1pct_usdt) {
            return {FilterReason::ThinOrderBook,
                    "scalp_gate: depth_1pct=" + std::to_string(static_cast<int64_t>(features.liquidity.book_depth_1pct_usdt)) +
                    " < min=" + std::to_string(static_cast<int64_t>(sp.hg_min_book_depth_1pct_usdt))};
        }
        if (features.orderbook.slippage_at_10usdt_bps > sp.hg_max_slippage_at_10usdt_bps) {
            return {FilterReason::ThinOrderBook,
                    "scalp_gate: slippage_$10=" + std::to_string(static_cast<int>(features.orderbook.slippage_at_10usdt_bps)) +
                    "bps > max=" + std::to_string(static_cast<int>(sp.hg_max_slippage_at_10usdt_bps))};
        }
        if (features.orderbook.resiliency_score > 0.0 &&
            features.orderbook.resiliency_score < sp.hg_min_resiliency) {
            return {FilterReason::ThinOrderBook,
                    "scalp_gate: resiliency=" + std::to_string(features.orderbook.resiliency_score) +
                    " < min=" + std::to_string(sp.hg_min_resiliency)};
        }
        double abs_funding = std::abs(snapshot.funding_rate);
        if (abs_funding > sp.hg_max_funding_abs) {
            return {FilterReason::HighTrapRisk,
                    "scalp_gate: funding_abs=" + std::to_string(abs_funding) +
                    " > max=" + std::to_string(sp.hg_max_funding_abs)};
        }
        if (features.volatility.realized_vol_1m_pct > 0.0 &&
            features.volatility.realized_vol_1m_pct < sp.hg_min_realized_vol_1m_pct) {
            return {FilterReason::LowVolatility,
                    "scalp_gate: vol_1m=" + std::to_string(features.volatility.realized_vol_1m_pct) +
                    "% < min"};
        }
        if (features.volatility.realized_vol_1m_pct > sp.hg_max_realized_vol_1m_pct) {
            return {FilterReason::ExtremeVolatility,
                    "scalp_gate: vol_1m=" + std::to_string(features.volatility.realized_vol_1m_pct) +
                    "% > max=" + std::to_string(sp.hg_max_realized_vol_1m_pct)};
        }
        if (features.trend_quality.wick_ratio > sp.hg_max_wick_ratio &&
            features.trend_quality.wick_ratio > 0.0) {
            return {FilterReason::HighNoise,
                    "scalp_gate: wick_ratio=" + std::to_string(features.trend_quality.wick_ratio) +
                    " > max=" + std::to_string(sp.hg_max_wick_ratio) + " (stop-hunt territory)"};
        }
    }
