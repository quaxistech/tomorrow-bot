...
RecoveryResult RecoveryService::recover_from_journal() {
    ...
    // Production-correctness fix (Phase 1.3): различаем "нет snapshot" (fresh deployment,
    // нормальный сценарий) от "snapshot unreadable" (I/O error, real failure).
    // Раньше оба попадали в Failed → ложный alarm на каждом first-run.
    auto snap_check = persistence_->snapshots().load_latest(persistence::SnapshotType::Portfolio);
    if (!snap_check.has_value()) {
        // Persistence пуст — fresh deployment, не ошибка.
        logger_->info(kComponent, "Снимок портфеля отсутствует — fresh start (recovery не требуется)");
        last_result_.messages.emplace_back("No snapshot found — fresh deployment");
        last_result_.status = RecoveryStatus::Completed;
    ...
    bool snapshot_ok = restore_from_snapshot();
    if (!snapshot_ok) {
        // Snapshot существует, но восстановление failed → real I/O / parse error.
        logger_->error(kComponent, "Snapshot существует, но не удалось восстановить (corrupted/parse error)");
        last_result_.messages.emplace_back("Failed to restore from existing snapshot");
        ++last_result_.errors;
        last_result_.status = RecoveryStatus::Failed;
    ...
    bool journal_ok = replay_journal_after_snapshot(snap_check->created_at);
    if (!journal_ok) {
