...
int64_t IdempotencyManager::now_ms() const {
    // Если injected clock доступен — используем его (testability).
    // Production-флоу всегда передаёт WallClock; тесты используют TestClock с
    // явным управлением временем (TestClock::current_time).
    //
    // Fallback на steady_clock сохраняется для случаев когда clock=nullptr
    // (защита от двойного fill при NTP-jump в legacy callsite, BUG-S34-05).
    if (clock_) {
        return clock_->now().get() / 1'000'000;  // ns → ms
    }
