    ...
    double uncertainty_size_multiplier)
{
    ...
    // Шаг 2: Fixed-margin sizing (EDGE-30-MARGIN run64 2026-05-15)
    // User explicit: bot должен фиксировать % от депозита как маржу, а изменять leverage
    // по сигналу. Не уменьшать size до min_notional при low budget.
    //
    // Formula:
    //   margin = total_capital × margin_pct (5%)
    //   notional = margin × max_leverage
    //   qty = notional / price
    //
    // Leverage scaled by conviction в leverage_engine отдельно (per-trade).
    // EDGE-30-MARGIN-v3 (run70): user issue — margin not stable across orders.
    // Fix: target leverage = max_leverage/2 (т.е. 37.5 при max=75). Это даёт более
    // стабильный target notional. Margin = 3% capital всегда.
    ...
    // Если strategy явно подсказала размер, и он больше — берём strategy hint
    // (для случаев high-conviction где нужно увеличить exposure).
    double strategy_hint = intent.suggested_quantity.get();
    if (strategy_hint > qty) {
        qty = strategy_hint;
    }

    if (qty <= 0.0) {
        result.approved = false;
        result.reduction_reason = "Нулевой объём (capital=0?)";
        return result;
    }
