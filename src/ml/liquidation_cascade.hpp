...
///
/// Научное обоснование (USDT-M perpetual futures):
/// - velocity_threshold=0.003 (0.3%): Brunnermeier & Pedersen (2009) показали,
///   что ценовые движения > 0.3% за тик сигнализируют о ликвидационном давлении
///   на рынках с высоким плечом. Для BTC/USDT perpetual средний тик ~0.01%,
///   30× стандартного движения = порог аномалии.
/// - volume_spike_mult=3.0: 3σ правило (3× от скользящего среднего) —
///   стандартный порог аномального объёма в market microstructure (Easley et al. 2012).
/// - depth_thin_threshold=0.3: стакан до 30% от нормы = дефицит ликвидности,
///   совпадает с порогами LOB-depletion в Cont et al. (2014).
/// - lookback=30: ~3 с при 10 тиков/с — компромисс между чувствительностью
///   и устойчивостью к одиночным выбросам.
/// - cascade_probability_threshold=0.6: выше 0.5 (случайное) + буфер 0.1
///   для снижения ложноположительных сигналов.
