...
/// - window_short=20: ~2 с при 10 тиков/с — захватывает краткосрочные
///   изменения корреляции (Epps effect corrected, Epps 1979).
/// - window_long=100: ~10 с — устойчивая базовая корреляция.
/// - decorrelation_threshold=0.3: |ρ| < 0.3 = слабая корреляция
///   (Cohen 1988, эффект средний при |r| ≥ 0.3).
/// - correlation_break_threshold=0.5: |Δρ| > 0.5 за короткий период =
///   структурный разрыв, типичный при ликвидационных событиях
///   (Forbes & Rigobon 2002).
/// - reference_assets: BTC и ETH как основные бенчмарки крипторынка.
