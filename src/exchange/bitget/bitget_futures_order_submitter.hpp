...
public:
    ...
    /// Pure-helper: построить JSON тело place-order запроса.
    /// Public для verification-tests (regression на Bitget side/holdSide convention).
    /// Не имеет сетевых side-effects.
    [[nodiscard]] std::string build_place_order_json(const execution::OrderRecord& order) const;
