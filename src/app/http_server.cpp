...
void HttpEndpointServer::stop() {
    ...
    // BUG-RUNTIME-1 fix: sync acceptor_->accept() блокирует worker_thread,
    // и close(acceptor) не всегда interrupt'ит на Linux (зависит от kernel/Boost).
    // Решение: (1) close acceptor; (2) self-connect чтобы разблокировать accept;
    // (3) join worker. Без этого SIGTERM-shutdown зависал на step 3.
    uint16_t port_snapshot = port_;
    {
        std::lock_guard lock(acceptor_mutex_);
        if (acceptor_ && acceptor_->is_open()) {
            boost::system::error_code ec;
    ...
    // Self-connect для wake-up — best-effort, ошибки игнорируем.
    try {
        boost::asio::io_context tmp_ctx;
        boost::asio::ip::tcp::socket s{tmp_ctx};
        boost::system::error_code ec;
        s.connect({boost::asio::ip::make_address("127.0.0.1"), port_snapshot}, ec);
        s.close(ec);
    } catch (...) {
        // best-effort
    }
