...
struct OrderWatchdogConfig {
    ...
    /// Максимальное время жизни открытого лимит-ордера (мс).
    /// run86 calibration: для scalping 30 sec слишком много — рынок уходит.
    /// 15 sec даёт limit-ордеру шанс зацепить fill при минорной волатильности,
    /// но не оставляет stale ордер при движении против.
    int64_t max_open_order_ms{15'000};
    /// Максимальное время частично исполненного ордера (мс)
    int64_t max_partial_fill_ms{30'000};
    ...
    /// run86 calibration: 10 sec слишком редко — за это время рынок может сильно
    /// уйти от цены лимита. 3 sec даёт быстрый отклик.
    int64_t check_interval_ms{3'000};
};
