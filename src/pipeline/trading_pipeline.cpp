    ...
    , metrics_(std::move(metrics))
{
    ...
    // D2 fix: фоновый пул для async-REST. 1 worker, очередь 32.
    // Все периодические REST-задачи (balance/funding/HTF/reconciliation) исполняются здесь.
    rest_pool_ = std::make_unique<RestWorkerPool>(
        /*worker_count=*/1, /*max_queue_depth=*/32, logger_, metrics_);
    ...
    risk_cfg.max_position_notional_pct = config_.risk.max_position_notional_pct;
...

    {
        ...
        // Clock sync check — fail-fast if time drift exceeds 30 seconds.
        // EDGE-29-MAX2: повышено 5000→30000 после run53 crash (drift -7824ms при 15+ paralleл
        // pipeline init вызывает REST latency spike). Real drift ≈700ms измерено отдельно.
        // 30s threshold — soft guard; Bitget сам отклонит signed orders с реально неправильным
        // временем. System NTP уже synced (timedatectl status: yes).
        try {
            int64_t offset_ms = rest_client->check_clock_sync();
            if (std::abs(offset_ms) > 30000) {
                throw std::runtime_error(
...
void TradingPipeline::set_kill_switch_provider(risk::KillSwitchProvider provider) {
    if (risk_engine_) {
        risk_engine_->set_kill_switch_provider(std::move(provider));
        logger_->info("pipeline",
            "D3: kill-switch provider установлен (Supervisor → RiskEngine)",
            {{"symbol", symbol_.get()}});
    }
}
...
void TradingPipeline::fetch_symbol_precision() {
    ...
    try {
        ...
        for (const auto& item : data) {
            ...
            // EDGE-30-DYN (run61): извлекаем maxLever per-symbol от Bitget
            double max_lev = 0.0;
            if (o.contains("maxLever")) {
                max_lev = parse_json_double(o.at("maxLever"));
                if (!std::isfinite(max_lev) || max_lev < 1.0) max_lev = 0.0;
            }
            ...
            logger_->info("pipeline", "Precision загружена с биржи",
                {{"symbol", sym},
                 ...
                 {"max_leverage_exchange", std::to_string(max_lev)}});
            return;
        }
...
void TradingPipeline::bootstrap_historical_candles() {
    ...
    htf_.last_update_ns = clock_->now().get();
}
...
void TradingPipeline::bootstrap_htf_candles() {
    ...
    try {
        ...
        if (closes.size() >= 50) {
            ...
            htf_.last_close = closes.back();

            // Сохраняем буферы для инкрементального обновления
            htf_.closes_buffer = closes;
            htf_.highs_buffer = highs;
            htf_.lows_buffer = lows;

            compute_htf_trend(closes);

            logger_->info("pipeline", "HTF-анализ завершён",
                {{"htf_candles", std::to_string(closes.size())},
                 {"htf_ema20", std::to_string(htf_.ema_20)},
                 {"htf_ema50", std::to_string(htf_.ema_50)},
                 {"htf_rsi", std::to_string(htf_.rsi_14)},
                 {"htf_adx", std::to_string(htf_.adx)},
                 {"htf_macd_hist", std::to_string(htf_.macd_histogram)},
                 {"htf_trend", std::to_string(htf_.trend_direction)},
                 {"htf_strength", std::to_string(htf_.trend_strength)}});
        }
...
void TradingPipeline::maybe_update_htf(const features::FeatureSnapshot& snapshot) {
    ...
    if (htf_.valid && snapshot.technical.atr_valid && htf_.last_close > 0.0) {
        double price_move = std::abs(snapshot.mid_price.get() - htf_.last_close);
        if (price_move > 3.0 * snapshot.technical.atr_14) {
            htf_.urgent_update_needed = true;
        }
    }

    // Регулярное обновление каждый час ИЛИ экстренное
    bool time_to_update = (htf_.last_update_ns == 0) ||
        ((now_ns - htf_.last_update_ns) >= HtfTrendState::kRefreshIntervalNs);

    if (!time_to_update && !htf_.urgent_update_needed) return;

    logger_->info("pipeline", "Обновление HTF данных",
        {{"reason", htf_.urgent_update_needed ? "URGENT: цена сдвинулась > 3×ATR" : "periodic_hourly"},
         {"symbol", symbol_.get()}});

    try {
        // Загружаем свежие свечи через REST (тот же метод что при bootstrap)
        bootstrap_htf_candles();
        htf_.last_update_ns = now_ns;

        // КРИТИЧНО: обновляем htf_.last_close текущей ценой, чтобы
        // urgent-проверка (price_move > 3×ATR) не срабатывала повторно.
        // bootstrap_htf_candles() ставит htf_.last_close = последняя часовая свеча,
        // которая может отличаться от real-time цены на > 3×ATR(1m).
        htf_.last_close = snapshot.mid_price.get();
        htf_.urgent_update_needed = false;
        ...
        htf_.last_update_ns = now_ns - HtfTrendState::kRefreshIntervalNs + 300'000'000'000LL; // Retry через 5 мин
        htf_.urgent_update_needed = false;
...
void TradingPipeline::compute_htf_trend(const std::vector<double>& closes) {
    ...
    // --- EMA20 и EMA50 через IndicatorEngine ---
    {
        ...
        if (ema20.valid) htf_.ema_20 = ema20.value;
        if (ema50.valid) htf_.ema_50 = ema50.value;
    }

    // --- RSI(14) через IndicatorEngine ---
    {
        auto rsi = indicator_engine_->rsi(closes, 14);
        if (rsi.valid) htf_.rsi_14 = rsi.value;
    }

    // --- MACD(12,26,9) через IndicatorEngine ---
    {
        auto macd = indicator_engine_->macd(closes, 12, 26, 9);
        if (macd.valid) htf_.macd_histogram = macd.histogram;
    }

    // --- ADX(14) через IndicatorEngine с реальными highs/lows ---
    {
        bool have_hlc = (htf_.highs_buffer.size() == n && htf_.lows_buffer.size() == n);
        if (have_hlc) {
            auto adx = indicator_engine_->adx(htf_.highs_buffer, htf_.lows_buffer, closes, 14);
            if (adx.valid) htf_.adx = adx.adx;
        } else {
    ...
    bool ema_bullish = htf_.ema_20 > htf_.ema_50;
    bool ema_bearish = htf_.ema_20 < htf_.ema_50;
    bool strong_trend = htf_.adx > 25.0;
    bool has_trend = htf_.adx > 20.0;

    if (ema_bullish && has_trend) {
        htf_.trend_direction = 1;
    } else if (ema_bearish && has_trend) {
        htf_.trend_direction = -1;
    } else {
        htf_.trend_direction = 0;
    }

    // Сила тренда: комбинация ADX + расстояния EMA + RSI
    double ema_gap_pct = (htf_.ema_50 > 0.0)
        ? std::abs(htf_.ema_20 - htf_.ema_50) / htf_.ema_50 * 100.0
        : 0.0;
    double adx_factor = std::clamp((htf_.adx - 15.0) / 35.0, 0.0, 1.0);
    double ema_factor = std::clamp(ema_gap_pct / 3.0, 0.0, 1.0);

    // RSI: чем дальше от 50, тем сильнее тренд
    double rsi_factor = std::clamp(std::abs(htf_.rsi_14 - 50.0) / 30.0, 0.0, 1.0);

    htf_.trend_strength = (adx_factor * 0.5 + ema_factor * 0.3 + rsi_factor * 0.2);
    htf_.trend_strength = std::clamp(htf_.trend_strength, 0.0, 1.0);

    htf_.valid = true;

    logger_->info("pipeline", "HTF тренд определён",
        {{"direction", htf_.trend_direction == 1 ? "UP" :
                       htf_.trend_direction == -1 ? "DOWN" : "SIDEWAYS"},
         {"strength", std::to_string(htf_.trend_strength)},
...
bool TradingPipeline::check_market_readiness(const features::FeatureSnapshot& snapshot) {
    ...
    if (htf_.rsi_14 < 10.0 || htf_.rsi_14 > 90.0) {
        if (market_ready_) {
            logger_->warn("pipeline",
    ...
    if (tick_count_ % 2000 == 0 && htf_.trend_strength > 0.5) {
        logger_->info("pipeline",
            "HTF тренд: направленный",
            {{"htf_trend", htf_.trend_direction == 1 ? "UP" :
                          htf_.trend_direction == -1 ? "DOWN" : "SIDEWAYS"},
             {"htf_strength", std::to_string(htf_.trend_strength)},
             {"htf_rsi", std::to_string(htf_.rsi_14)}});
    }
...
bool TradingPipeline::check_hedge_recovery(const features::FeatureSnapshot& snapshot) {
    ...
    input.funding_rate = funding_tracker_.current_rate();
            ...
            if (is_full_close) {
...

                {
                    double sl_pnl_pct = (pos.notional.get() > 0.0)
                        ? pos.unrealized_pnl / pos.notional.get() * 100.0
                        : 0.0;
                    trade_stats_.record(sl_pnl_pct);
                }
...
bool TradingPipeline::start() {
    ...
    // Фьючерсы: установка margin mode, hold mode и leverage на бирже ПЕРЕД первым ордером.
    // D4 fail-fast: при отказе любой из 4 проверок pipeline отказывается стартовать —
    // продолжать торговлю со mismatched leverage / one-way hold mode опасно.
    if (futures_submitter_) {
        // 1. Position mode = hedge_mode (long + short одновременно)
        if (!futures_submitter_->set_hold_mode(
                config_.futures.product_type, "hedge_mode")) {
            logger_->critical("pipeline",
                "D4: не удалось установить hold_mode=hedge_mode на бирже. "
                "Pipeline отказывается стартовать (Inv-11 violation).",
                {{"product_type", config_.futures.product_type},
                 {"symbol", symbol_.get()}});
            return false;
        }

        // 2. Margin mode (isolated/crossed) для текущего символа
        if (!futures_submitter_->set_margin_mode(
                symbol_, config_.futures.margin_mode)) {
            logger_->critical("pipeline",
                "D4: не удалось установить margin_mode на бирже. "
                "Pipeline отказывается стартовать (Inv-11 violation).",
        ...
        // 3. Установка leverage — EDGE-30-DYN: use min(config, exchange_per_symbol)
        // Bitget per-symbol max_leverage varies (BTC 125x, alts 20-75x). Берём minimum.
        int effective_lev = config_.futures.default_leverage;
        if (exchange_rules_.max_leverage > 0.0) {
            int exch_max = static_cast<int>(exchange_rules_.max_leverage);
            if (exch_max < effective_lev) {
                logger_->info("pipeline", "Leverage скорректирован под exchange лимит",
                    {{"requested", std::to_string(config_.futures.default_leverage)},
                     {"exchange_max", std::to_string(exch_max)},
                     {"symbol", symbol_.get()}});
                effective_lev = exch_max;
            }
        }
        if (!futures_submitter_->set_leverage(symbol_, effective_lev, "long")) {
            logger_->critical("pipeline",
                "D4: не удалось установить leverage для long-стороны. "
                "Pipeline отказывается стартовать (Inv-11 violation).",
                {{"leverage", std::to_string(effective_lev)},
                 {"symbol", symbol_.get()}});
            return false;
        }
        if (!futures_submitter_->set_leverage(symbol_, effective_lev, "short")) {
            logger_->critical("pipeline",
                "D4: не удалось установить leverage для short-стороны. "
                "Pipeline отказывается стартовать (Inv-11 violation).",
                {{"leverage", std::to_string(effective_lev)},
                 {"symbol", symbol_.get()}});
            return false;
        ...
        logger_->info("pipeline", "Фьючерсный режим настроен (D4 fail-fast verified)",
    ...
    if (futures_query_adapter_) {
        const double startup_funding = futures_query_adapter_->get_current_funding_rate(symbol_);
        funding_tracker_.update(startup_funding, clock_->now().get());
        if (risk_engine_) {
            risk_engine_->set_funding_rate(startup_funding);
        }
    }
    ...
    // on the first live tick; spread the periodic jobs across symbols instead.
    {
        ...
        // Funding rate timestamp хранится в FundingRateTracker; spread offset тут не нужен,
        // т.к. tracker инициализирован 0 и отработает сразу в первом цикле periodic-tasks.
...
void TradingPipeline::stop() {
    ...
    // D2: останавливаем фоновый REST-пул ДО уничтожения engines, чтобы
    // in-flight async tasks не обращались к уже разрушенным объектам.
    if (rest_pool_) {
        rest_pool_->drain(std::chrono::milliseconds(2000));
        rest_pool_.reset();
    }
...
void TradingPipeline::on_feature_snapshot(features::FeatureSnapshot snapshot) {
    ...
    if (!pre_trade_has_position &&
        ...
        if (market_reaction_engine_) {
            cached_market_state_ = market_reaction_engine_->build_state(
                ...
                funding_tracker_.current_rate(),
                htf_.trend_direction * htf_.trend_strength,
                htf_.valid,
    ...
    const portfolio::Position* short_position = nullptr;
    {
        ...
        for (const auto& strat : strategy_registry_->active()) {
            ...
            ctx.htf_trend_direction = htf_.trend_direction;
            ctx.htf_trend_strength = htf_.trend_strength;
    ...
    if (snapshot.technical.atr_valid && snapshot.technical.atr_14 > 0.0 &&
        ...
        if ((intent.side == Side::Buy && htf_.trend_direction == 1) ||
            (intent.side == Side::Sell && htf_.trend_direction == -1)) {
            reward_boost *= 1.20;
        }
    ...
    // иначе бот не сможет зафиксировать прибыль в аптренде.
        ...
        bool blocked = false;
        // EDGE-21 (2026-05-14): HTF threshold 0.15 → 0.50 — был слишком жёсткий,
        // блокировал большинство SELL setups в умеренном aptrendу.
        if (intent.side == Side::Buy && htf_.trend_direction == -1 && htf_.trend_strength > 0.50) {
            // Allow only if clear reversal: MACD > 0 AND RSI > 40
            bool reversal_confirmed = (htf_.macd_histogram > 0.0 && htf_.rsi_14 > 40.0);
            if (!reversal_confirmed && !is_close_order) {
                ...
                if (tick_count_ % 200 == 0) {
                    logger_->warn("pipeline",
                        ...
                        {{"htf_trend", std::to_string(htf_.trend_direction)},
                         {"htf_strength", std::to_string(htf_.trend_strength)},
                         {"htf_macd", std::to_string(htf_.macd_histogram)},
                         {"htf_rsi", std::to_string(htf_.rsi_14)},
                         {"strategy", intent.strategy_id.get()}});
                }
            }
        }
        // EDGE-21: HTF threshold 0.15 → 0.50 для SELL — было overblocking.
        if (intent.side == Side::Sell && !is_close_order && htf_.trend_direction == 1 && htf_.trend_strength > 0.50) {
            bool reversal_confirmed = (htf_.macd_histogram < 0.0 && htf_.rsi_14 < 60.0);
            ...
            if (diag_htf_block_ <= kDiagLogLimit) {
                logger_->info("pipeline", "HTF Trend Filter блокирует сигнал",
                    {{"side", intent.side == Side::Buy ? "BUY" : "SELL"},
                     {"htf_direction", std::to_string(htf_.trend_direction)},
                     {"htf_strength", std::to_string(htf_.trend_strength)},
    ...
    if (is_closing_position) {
        ...
        // Volatility Targeting: передаём рыночный контекст в аллокатор
        {
            ...
            double win_rate = trade_stats_.win_rate();
            double win_loss_ratio = trade_stats_.win_loss_ratio();
        ...
        alloc_ctx.win_rate = trade_stats_.win_rate();
        alloc_ctx.avg_win_loss_ratio = trade_stats_.win_loss_ratio();
        ...
        // EDGE-30-MARGIN-v4 (run75): user issue — margin не было FIXED 3% от full balance.
        // Раньше capital делили на num_pipelines → margin становилась мизерной → exchange
        // bumped notional до $5 min → effective margin 5.5%. Используем FULL capital
        // для margin calc; concurrency limits управляются max_concurrent_positions.
    ...
    if (leverage_engine_) {
        ...
        lev_ctx.funding_rate = funding_tracker_.current_rate();
        ...
        // EDGE-16 (run31 SAGA liquidation -$4.11): hard guard против pyramiding позиций.
        // DualLegManager инициализируется на каждой scan rotation (10мин), теряет position state.
        // Multiple Open intents accumulate без знания об existing exchange position.
        // Запрещаем Open если уже есть position (любой size > min_threshold).
        if (intent.trade_side == TradeSide::Open && has_open_position()) {
            logger_->warn("pipeline", "BLOCK Open: already have local position (anti-pyramiding EDGE-16)",
                {{"symbol", symbol_.get()},
                 {"current_size", std::to_string(initial_position_size_)},
                 {"current_side", current_position_side_ == PositionSide::Long ? "long" : "short"},
                 {"intent_side", intent.position_side == PositionSide::Long ? "long" : "short"}});
            return;
        }
        ...
        if (intent.trade_side == TradeSide::Open) {
            ...
            if (target_margin > 0.0 && actual_notional > 0.0) {
                int required_lev = static_cast<int>(std::ceil(actual_notional / target_margin));
                int exch_max = (exchange_rules_.max_leverage > 0.0)
                    ? static_cast<int>(exchange_rules_.max_leverage)
                    : config_.futures.max_leverage;
                int auto_lev = std::clamp(required_lev,
                                          lev_decision.leverage,
                                          std::min(config_.futures.max_leverage, exch_max));
                if (auto_lev > lev_decision.leverage) {
                    logger_->info("pipeline", "Leverage авто-подъём для FIXED margin 3%",
                        {{"symbol", symbol_.get()},
                         {"computed_leverage", std::to_string(lev_decision.leverage)},
                         {"auto_leverage", std::to_string(auto_lev)},
                         {"actual_notional", std::to_string(actual_notional)},
                         {"target_margin", std::to_string(target_margin)},
                         {"effective_margin_pct", std::to_string(100.0 * actual_notional / auto_lev / pre_trade_portfolio.total_capital)}});
                    lev_decision.leverage = auto_lev;
                }
            }

            // EDGE-30-LEV-LOG (run67): user wants видеть актуальное dynamic leverage.
            logger_->info("pipeline", "Dynamic leverage decision",
                {{"symbol", symbol_.get()},
                 {"computed_leverage", std::to_string(lev_decision.leverage)},
                 {"last_applied", std::to_string(last_applied)},
                 {"will_update", std::abs(lev_decision.leverage - last_applied) >= min_delta ? "yes" : "no"},
                 {"hold_side", hold_side}});
    ...
    if (order_result) {
        ...
        if (is_position_close) {
            ...
            trade_stats_.record(closing_pnl_pct);
...
void TradingPipeline::run_periodic_tasks(int64_t now_ns) {
    ...
    // Production hardening: pipeline → RiskEngine — stale-state guards.
    // RiskEngine::evaluate() прочитает эти флаги через RiskContext и заблокирует
    // entry-ордера пока система не вернётся в healthy state.
    if (risk_engine_) {
        risk_engine_->set_reconciliation_desync(
            reconciliation_needs_resync_.load(std::memory_order_acquire));
        risk_engine_->set_ws_disconnected(!is_connected());
    }

    // D2 fix: reconciliation вынесена в RestWorkerPool — REST не блокирует hot-path.
    // Hot-path только публикует задачу; интервалы поддерживаются `last_reconciliation_ns_`.
    if (reconciliation_engine_ && rest_pool_ &&
        (last_reconciliation_ns_ == 0 ||
         (now_ns - last_reconciliation_ns_) >= kReconciliationIntervalNs)) {
        last_reconciliation_ns_ = now_ns;
        bool expected = false;
        if (reconciliation_in_flight_.compare_exchange_strong(expected, true, std::memory_order_acq_rel)) {
            rest_pool_->submit("reconciliation", [this, now_ns] {
                try {
                    run_continuous_reconciliation(now_ns);
                } catch (const std::exception& e) {
                    logger_->error("pipeline",
                        "reconciliation async упала с исключением",
                        {{"what", e.what()}, {"symbol", symbol_.get()}});
                }
                reconciliation_in_flight_.store(false, std::memory_order_release);
            });
        }
    }
    ...
    // Production hardening 2026-05-09: периодический re-sync clock с биржей.
    // Bitget recvWindow ~5 сек; NTP drift в long-running session может сломать подписи.
    // Async через rest_pool_ — hot-path не блокируется.
    if (rest_client_ && rest_pool_ &&
        (last_clock_sync_ns_ == 0 ||
         (now_ns - last_clock_sync_ns_) >= kClockSyncIntervalNs)) {
        last_clock_sync_ns_ = now_ns;
        bool expected = false;
        if (clock_sync_in_flight_.compare_exchange_strong(expected, true, std::memory_order_acq_rel)) {
            rest_pool_->submit("clock_sync", [this] {
                try {
                    const int64_t offset_ms = rest_client_->check_clock_sync();
                    if (std::abs(offset_ms) > 2000) {
                        logger_->critical("pipeline",
                            "Clock drift > 2s — все ордера будут rejected биржей",
                            {{"offset_ms", std::to_string(offset_ms)},
                             {"symbol", symbol_.get()}});
                        // Помечаем degraded через risk engine — блокирует новые entries.
                        if (risk_engine_) {
                            risk_engine_->set_ws_disconnected(true);
                        }
                    }
                } catch (const std::exception& e) {
                    logger_->error("pipeline",
                        "clock_sync async упал с исключением",
                        {{"what", e.what()}, {"symbol", symbol_.get()}});
                }
                clock_sync_in_flight_.store(false, std::memory_order_release);
            });
        }
    }

    // D2 fix: Периодическая синхронизация баланса — async через RestWorkerPool.
    // Hot-path только публикует задачу; REST отрабатывает в фоне.
    if (rest_client_ && rest_pool_ &&
        (last_balance_sync_ns_ == 0 ||
         (now_ns - last_balance_sync_ns_) >= kBalanceSyncIntervalNs ||
         reconciliation_needs_resync_.load(std::memory_order_acquire))) {
        const bool resync_flag = reconciliation_needs_resync_.exchange(false, std::memory_order_acq_rel);
        last_balance_sync_ns_ = now_ns;
        bool expected = false;
        if (balance_sync_in_flight_.compare_exchange_strong(expected, true, std::memory_order_acq_rel)) {
            rest_pool_->submit("balance_sync", [this, resync_flag] {
                if (resync_flag) {
                ...
                try {
                    ...
                    logger_->error("pipeline",
                        "balance_sync async упала с исключением",
                        {{"what", e.what()}, {"symbol", symbol_.get()}});
                }
                balance_sync_in_flight_.store(false, std::memory_order_release);
            });
        }
    }

    // D2/D6 fix: Периодическое обновление funding rate — async через RestWorkerPool.
    // Интервал и in-flight живут в `funding_tracker_` (см. FundingRateTracker).
    if (futures_query_adapter_ && rest_pool_ && funding_tracker_.should_refresh(now_ns)) {
        if (funding_tracker_.try_begin_refresh()) {
            rest_pool_->submit("funding_rate_update", [this] {
                try {
                    const double new_rate = futures_query_adapter_->get_current_funding_rate(symbol_);
                    const double old_rate = funding_tracker_.current_rate();
                    const bool changed = funding_tracker_.update(new_rate, clock_->now().get());
                    if (changed) {
                        logger_->info("pipeline", "Funding rate обновлён",
                            {{"symbol", symbol_.get()},
                             {"old_rate", std::to_string(old_rate)},
    ...
    {
        const double funding_now = funding_tracker_.current_rate();
        if (obs_panels_.funding_rate_bps && std::abs(funding_now) > 1e-10) {
            obs_panels_.funding_rate_bps->observe(funding_now * 10000.0); // rate → bps
            if (obs_panels_.funding_drag_cumulative_bps && has_open_position()) {
                obs_panels_.funding_drag_cumulative_bps->increment(
                    std::abs(funding_now) * 10000.0);
            }
        }
    }
...
void TradingPipeline::run_continuous_reconciliation(int64_t now_ns) {
    // D2: периодичность контролируется в `run_periodic_tasks` (caller).
    // Эта функция теперь либо вызвана из RestWorkerPool worker'а (async),
    // либо напрямую при manual-resync. Inner-gate удалён, чтобы не блокировать
    // выполнение задачи которая только что была запланирована.
            ...
            for (const auto& mismatch : result.mismatches) {
                ...
                switch (mismatch.resolved_by) {
                    ...
                    case reconciliation::ResolutionAction::SyncFromExchange: {
                        ...
                        reconciliation_needs_resync_.store(true, std::memory_order_release);
                        logger_->info("reconciliation",
...
// ============================================================
// Rolling Trade Statistics — D6: вынесено в TradingHistoryStats
// (см. pipeline/trading_history_stats.hpp). Старая реализация удалена.
// ============================================================
...
ExitContext TradingPipeline::build_exit_context(
    ...
    ctx.funding_rate = funding_tracker_.current_rate();
