...
ExitDecision PositionExitOrchestrator::evaluate(const ExitContext& ctx) const {
    ...
    // EDGE-30-PROFIT-COMBO (run81 analysis 2026-05-15): combined approach.
    // User analysis: run79 closed 4 winners at exactly 5% margin ROI (TP threshold).
    // Winners could potentially run further. New strategy:
    //   - При pnl >= 0.30% notional (≈5% margin ROI lev=15): LOCK profit но НЕ закрывать всё.
    //     Activate aggressive trailing stop (handled by trail logic ниже via profit_f scale).
    //   - При pnl >= 0.60% notional (≈10% margin ROI): force close ВСЕЙ позиции (high-conviction harvest).
    // Между этими — trailing stop captures runs, partial_tp_atr (4×ATR) takes 33% chunks.
    if (ctx.position_size > 0.0 && ctx.current_price > 0.0) {
        double notional = ctx.position_size * ctx.current_price;
        // EDGE-30-COMBO: force close ТОЛЬКО при very high profit (10%+ margin ROI).
        // Раньше 5% закрывало всё — ловили small win, теряли upside.
        // Теперь 5% → trailing активируется (profit_f ниже в коде scales trail wider).
        // 10% margin ROI = ~0.60% notional → force close (high reversal risk на extreme).
        double force_tp_threshold_high = notional * 0.006;  // 10% margin ROI
        if (ctx.unrealized_pnl >= force_tp_threshold_high) {
            ExitDecision d;
            d.should_exit = true;
            d.urgency = 0.95;
            d.explanation.primary_signal = ExitSignalType::QuickProfitHarvest;
            d.explanation.category = ExitCategory::Alpha;
            d.explanation.primary_driver = std::format(
                "EDGE-30-COMBO force TP HIGH: pnl={:.4f} >= {:.4f} (~10% margin ROI)",
                ctx.unrealized_pnl, force_tp_threshold_high);
            d.explanation.counterfactual = "Continue if profit below high-margin-ROI threshold";
            return d;
        }
        // Note: 5% margin ROI больше НЕ force close. Trailing stop + partial TP управляют дальше.
    }

    // EDGE-17 + EDGE-27 (extended adverse exit 2026-05-15): tiered fast exits.
    // EDGE-17: hold<60s + pnl<-0.4% → exit (pre-reversal entries)
    // EDGE-27: hold<5min + pnl<-0.6% → exit (medium-term failures, catches RIVER-type -$0.05 losses)
    // EDGE-27: hold<10min + pnl<-0.8% → exit (longer holds где CONTINUATION слишком медленный)
    {
        int64_t hold_ns = ctx.now_ns - ctx.entry_time_ns;
        const double pnl_pct = ctx.unrealized_pnl_pct;

        // EDGE-30-LOSSCUT (run81 analysis 2026-05-15): user audit — SAGA loss -0.015 USDT
        // closed by EMA-cross при -0.72% pnl (hold 6 min) — выпал между EDGE-30 (<5min/-1.0%)
        // и EMA структурным exit. Tighter mid-tier чтобы catch такие losses раньше.
        // Также скальпинг с small wins требует tight stops для favorable expectancy.
        bool fast_adverse = false;
        const char* tier = "";
        if (hold_ns > 5'000'000'000LL && hold_ns < 60LL * 1'000'000'000LL && pnl_pct < -0.5) {
            fast_adverse = true; tier = "EDGE-30 <60s/-0.5%";   // 0.7→0.5: tighter early
        } else if (hold_ns < 300LL * 1'000'000'000LL && pnl_pct < -0.7) {
            fast_adverse = true; tier = "EDGE-30 <5min/-0.7%";  // 1.0→0.7: SAGA-type catch
        } else if (hold_ns < 600LL * 1'000'000'000LL && pnl_pct < -0.9) {
            fast_adverse = true; tier = "EDGE-30 <10min/-0.9%"; // 1.3→0.9: medium hold
        } else if (hold_ns < 1800LL * 1'000'000'000LL && pnl_pct < -1.2) {
            fast_adverse = true; tier = "EDGE-30 <30min/-1.2%"; // new tier: long hold protection
        }
        ...
        if (fast_adverse) {
            ...
            d.should_exit = true;
            d.urgency = 0.95;
            d.explanation.primary_signal = ExitSignalType::HardRiskStop;
            d.explanation.category = ExitCategory::RiskKill;
            d.explanation.primary_driver = std::format(
                "{}: hold={:.0f}s pnl_pct={:.2f}%",
                tier, hold_ns / 1e9, pnl_pct);
            d.explanation.counterfactual = "Hold longer only if adverse move stalls";
            return d;
        }
    }
...
ExitDecision PositionExitOrchestrator::check_funding_carry_exit(const ExitContext& ctx) const {
    ...
    // EDGE-12 (live run25 INTC 1s exit @ funding=0.1%/8h=109%APR > 100% threshold):
    // Funding settles only на 8h boundary. Если hold_time < 4ч (240мин), вероятность
    // pay funding = hold_min/480 < 50%. Bot теряет $0.003 + fees instant вместо risk
    // 6% chance заплатить funding. Skip funding-carry exit для scalp hold times.
    int64_t hold_ns = ctx.now_ns - ctx.entry_time_ns;
    constexpr int64_t kMinHoldForFundingCheck = 4LL * 3600LL * 1'000'000'000LL; // 4 hours
    if (hold_ns < kMinHoldForFundingCheck) return {};

    // Annualized cost: funding × 3 (per day) × 365
    double annualized_pct = effective_funding * 100.0 * 3.0 * 365.0;

    // BUG-EDGE-5 → 12: при hold ≥ 4ч уже близко к funding boundary, разумно exit
    // если deathspiral funding. Threshold 100% loss / 200% win.
    double threshold_annualized = (ctx.unrealized_pnl_pct < 0.0) ? 100.0 : 200.0;
...
ExitDecision PositionExitOrchestrator::check_continuation_value_exit(
    ...
    // EDGE-29 (run48 2026-05-15): расширили 1.5x → 3.0x — run48 HYPEUSDT 2 сделки подряд закрылись
    // continuation_exit при PnL ~-0.011 (на 12% ниже 1.5×fee floor). 3.0× fee создаёт adequate
    // noise band для микро-account где raw market noise сопоставим с fee burden.
    double shallow_loss_floor = -round_trip_fee * 3.0;
    ...
    // EDGE-30-CONT-v2 (run73): требуем 3+ bearish_confirms (раньше 2), cont threshold -0.40.
    // CHIP в run73 exited @ cont=-0.29 с 2 confirms → недостаточно. 3 confirms = stronger signal.
    if (bearish_confirms < 3 && state.continuation_value > -0.40) return {};
