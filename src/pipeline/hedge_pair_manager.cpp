...
void HedgePairManager::confirm_hedge_filled() {
    // Two-phase commit: explicit подтверждение fill'а от portfolio.
    // Идемпотентно: повторный вызов из PrimaryPlusHedge — no-op.
    if (state_ == HedgePairState::HedgeSentPending) {
        ++hedge_count_;
        state_ = HedgePairState::PrimaryPlusHedge;
        if (logger_) {
            logger_->info("pipeline", "HEDGE confirmed (explicit) — entering pair mode",
                {{"hedge_count", std::to_string(hedge_count_)}});
        }
    } else if (state_ == HedgePairState::PrimaryOnly) {
        // Из PrimaryOnly прямой commit допустим (legacy behavior из тестов).
        ++hedge_count_;
        state_ = HedgePairState::PrimaryPlusHedge;
    }
}
