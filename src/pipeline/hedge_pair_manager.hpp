...
public:
    ...
    /// Переводит state → HedgeSentPending. Не инкрементирует hedge_count_.
    void notify_hedge_opened();

    /// Подтверждение: portfolio зарегистрировал fill хедж-ноги.
    /// Two-phase commit: только этот вызов (или evaluate(input.has_hedge=true))
    /// инкрементирует hedge_count_ и переводит в PrimaryPlusHedge.
    /// Защита от учёта rejected/orphan ордеров.
    void confirm_hedge_filled();
