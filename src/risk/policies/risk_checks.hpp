...
/// 36. Reconciliation desync — блокирует НОВЫЕ entries при mismatch с биржей.
/// Close/reduce разрешены (для сокращения exposure). Active при `ctx.reconciliation_desync`.
class ReconciliationDesyncCheck : public IRiskCheck {
public:
    ReconciliationDesyncCheck() = default;
    std::string_view name() const noexcept override { return "reconciliation_desync_check"; }
    void evaluate(const RiskContext& ctx, RiskDecision& decision) override;
};

/// 37. WS disconnected — блокирует НОВЫЕ entries при отвале public WS.
/// Без свежего стакана/тиков торговля опасна (price может быть устаревшей).
/// Close/reduce также блокируются (нельзя закрыть без актуальной цены).
class WsDisconnectedCheck : public IRiskCheck {
public:
    WsDisconnectedCheck() = default;
    std::string_view name() const noexcept override { return "ws_disconnected_check"; }
    void evaluate(const RiskContext& ctx, RiskDecision& decision) override;
};
