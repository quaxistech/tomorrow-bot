...
struct RiskContext {
    ...
    /// Reconciliation engine обнаружила mismatch с биржей и ждёт ресинхронизации.
    /// Любые НОВЫЕ entry-ордера должны блокироваться; close/reduce разрешены для
    /// сокращения exposure. Pipeline выставляет из `reconciliation_needs_resync_`.
    bool reconciliation_desync{false};
    /// WS-канал недоступен / отвалился. Все новые entry-ордера блокируются.
