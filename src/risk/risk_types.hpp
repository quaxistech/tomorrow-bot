...
struct ExtendedRiskConfig {
    // === Per-trade ===
    double max_position_notional{10000.0};      ///< Макс. номинал одной позиции (USDT, абсолютный floor)
    double max_position_notional_pct{0.0};      ///< Макс. номинал как доля от total_capital (0=отключено, >0 auto-scales)
    ...
    // EDGE-19 (opportunity flow recovery 2026-05-14): 1 loss + 5 min cooldown был
    // слишком агрессивен — после fast adverse exit (EDGE-17) bot простаивал 16 мин.
    // 2 losses + 2 min: 1 loss = шум, 2 losses подряд = реально плохой symbol.
    int max_consecutive_losses_per_symbol{2};   ///< 1 → 2 (1 loss не блокирует)
    int64_t symbol_cooldown_after_stopouts_ns{120'000'000'000LL}; ///< 5 → 2 мин
    ...
    // === Positions ===
    // EDGE-30-LIMIT (run62 2026-05-15): hardcoded limits 5/3/3 блокировали 4+ positions.
    // User wants больше параллельных trades. Поднимаем до 10/8/8.
    int max_concurrent_positions{10};           ///< Макс. одновременных позиций
    int max_simultaneous_long_positions{8};     ///< Макс. одновременных long
    int max_simultaneous_short_positions{8};    ///< Макс. одновременных short
    ...
    // BUG-EDGE-4 (run14): 30% APR блокировал 19 entries, но scalping hold time ~5min →
    // funding pays только если позиция переходит через 8h boundary. Поднято до 200% —
    // все ещё фильтрует deathspiral pairs (>0.18% per 8h), но допускает обычные alts.
    double max_annual_funding_cost_pct{200.0};  ///< 30 → 200% — scalping не платит funding обычно
