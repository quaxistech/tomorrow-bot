...
void ProductionRiskEngine::init_checks() {
    ...
    // === Production hardening: stale-state submit guards ===
    checks_.push_back(std::make_unique<ReconciliationDesyncCheck>());      // 36
    checks_.push_back(std::make_unique<WsDisconnectedCheck>());            // 37
}
    ...
    const uncertainty::UncertaintySnapshot& uncertainty)
{
    ...
    // D3: синхронизация с authoritative kill-switch provider (Supervisor).
    // На каждом evaluate перечитываем состояние, не полагаясь только на listener.
    // Закрывает race window между `Supervisor::activate_global_kill_switch`
    // и доставкой в local listener.
    if (kill_switch_provider_) {
        const auto [external_active, external_reason] = kill_switch_provider_();
        if (external_active) {
            kill_switch_active_.store(true);
            if (!state_.locks.has_emergency_halt()) {
                kill_switch_reason_ = external_reason;
                state_.locks.add_lock(LockType::EmergencyHalt, "",
                    external_reason.empty() ? std::string{"supervisor_kill_switch"} : external_reason,
                    clock_->now());
            }
        } else if (kill_switch_active_.load()) {
            // Authoritative источник снял kill switch — синхронизируем local state.
    ...
    ctx.reconciliation_desync = reconciliation_desync_.load(std::memory_order_acquire);
    ctx.ws_disconnected = ws_disconnected_.load(std::memory_order_acquire);
...
bool ProductionRiskEngine::is_kill_switch_active() const {
    // D3: aggregate local + authoritative source (если установлен).
    if (kill_switch_active_.load()) {
        return true;
    }
    if (kill_switch_provider_) {
        return kill_switch_provider_().first;
    }
    return false;
}

void ProductionRiskEngine::set_kill_switch_provider(KillSwitchProvider provider) {
    std::lock_guard lock(mutex_);
    kill_switch_provider_ = std::move(provider);
}
...
void ProductionRiskEngine::set_reconciliation_desync(bool desync) {
    reconciliation_desync_.store(desync, std::memory_order_release);
}

void ProductionRiskEngine::set_ws_disconnected(bool disconnected) {
    ws_disconnected_.store(disconnected, std::memory_order_release);
}
