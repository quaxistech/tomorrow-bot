...
void Supervisor::signal_handler(int sig) {
    // ASYNC-SIGNAL-SAFE: только atomic CAS + write(), без logger/malloc/mutex/CV.
    //
    ...
    (void)sig;
    // Async-safe debug trace (write to stderr fd=2).
    static const char trace[] = "[supervisor.signal_handler] received\n";
    ::write(2, trace, sizeof(trace) - 1);

    Supervisor* sv = instance_.load(std::memory_order_acquire);
    if (sv == nullptr) {
        static const char miss[] = "[supervisor.signal_handler] no instance\n";
        ::write(2, miss, sizeof(miss) - 1);
        return;
    }
    ...
    bool ok = sv->state_.compare_exchange_strong(running,  shutting, std::memory_order_acq_rel) ||
    ...
    if (ok) {
        static const char ok_msg[] = "[supervisor.signal_handler] state -> ShuttingDown\n";
        ::write(2, ok_msg, sizeof(ok_msg) - 1);
    } else {
        static const char fail[] = "[supervisor.signal_handler] CAS failed (unexpected state)\n";
