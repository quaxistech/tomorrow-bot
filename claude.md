...
**Дата:** 2026-05-08 (production hardening pass 2 завершён)
**Объём:** 37 модулей в `src/` (после удаления 5 orphan-каталогов), C++23  
...
* единственная стратегия (скальпинг) с 4 внутренними сетапами; «множество стратегий» в архитектуре сохранено как точка расширения, но фактически имеется одна
...
* Catch2 v3 (FetchContent) — тестовый фреймворк, 556 тестов
...
| 7 | Принятие решений | `strategy/` (+ `setups`, `state`, `context`, `management`), `strategy_allocator/`, `decision/`, `ml/` |
...
| `world_model` | 9 состояний рынка с гистерезисом, fragility, persistence, transition context, feedback по стратегиям |
| `regime` | 13 detailed regimes, hysteresis (dwell/candidate ticks), CUSUM-ускорение, regime hints |
| `uncertainty` | 9-мерная неопределённость (regime, signal, data quality, exec, portfolio, ML, correlation, transition, operational) |
| `strategy` | Скальпинг: state machine, setup detector/validator, position manager, 4 сетапа |
| `strategy_allocator` | Аллокация веса между стратегиями по режиму (фактически 1 стратегия) |
| `decision` | Committee voting, conviction, drawdown boost, ensemble bonus, time decay, execution-cost penalty |
...
| `execution` | `ExecutionEngine` + 6 подсистем (registry, planner, fill_processor, cancel_manager, recovery_manager, exec_metrics), FSM, TWAP |
...
| `leverage` | Адаптивное плечо: 7 множителей (regime, vol, drawdown, conviction, funding, adversarial, uncertainty), Kelly cap, EMA smoothing |
...
Уровень 4: `ml`, `strategy` (+ sub), `strategy_allocator`, `decision`.
